  ...
  try {
    ...
    const fetchReviews = async (datasetId, source) => {
      ...
      for (const item of items) {
        ...
        // ✅ FIX 1: always check flat format (no "arr.length === 0")
    ...
    try {
      ...
      if (yRes.ok) {
        ...
        console.log('Yelp API error:', await yRes.text());
      }
    ...
    try {
      ...
      if (tRes.ok) {
        ...
        console.log('TripAdvisor API error:', await tRes.text());
      }
    ...
    console.log('Started — Google:', gData?.data?.id, 'Yelp:', yelpRunId, 'TripAdvisor:', tripRunId);
